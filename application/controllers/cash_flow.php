...
class cash_flow extends CI_Controller {
    ...
    function simpan()
    {
        ...
        if($tipe == 1){ //saldo awal

            $cek = $this->model_main->data_result('cash_flow',array('month(tanggal)' => $bulan, 'year(tanggal)'=>$tahun),'delete_by IS NULL'); //jika bulan ini sudah input saldo
            if($cek->num_rows() > 0){
